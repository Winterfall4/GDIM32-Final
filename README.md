...
### Team Member Gianine Ariane Umali
Since the check-in I have mostly worked on the dialogue aspect of the game. I wrote all the dialogue that is seen in the game. Kai first implemented the dialogue system based on what we learned in the class. Then I adjusted the classes to meet the needs of our game. For example, I moved all the logic that Kai implemented to a parent class called Dialogue which has the logic that we learned in class like going from one text to the next. But then I made children classes of that Dialogue class for each NPC like the dog and crush and adjusted and added to the code so that it would work the way that we wanted to. For example, I worked on the Crush class which is a child of the Dialogue class. I also worked on the Dog Dialogue class that Kai started.
